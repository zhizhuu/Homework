...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_kernel = torch.tensor([[0, 1, 0],
                                     [1, -4, 1],
                                     [0, 1, 0]], dtype=torch.float32, device=foreground_img.device).unsqueeze(
        0).unsqueeze(0)

    # Repeat the kernel for each channel
    laplacian_kernel = laplacian_kernel.repeat(foreground_img.size(1), 1, 1, 1)

    # Apply the Laplacian filter to both images
    foreground_laplacian = F.conv2d(foreground_img, laplacian_kernel, groups=foreground_img.size(1), padding=1)
    blended_laplacian = F.conv2d(blended_img, laplacian_kernel, groups=blended_img.size(1), padding=1)

    # Compute the Laplacian loss within the masks
    combined_mask = foreground_mask * background_mask
    loss = ((foreground_laplacian - blended_laplacian) ** 2) * combined_mask
    loss = loss.sum()
